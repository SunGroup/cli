    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "Spam Is NOT Allowed Here\n@"..username.." ["..msg.from.id.."]\n\n Status: User Kicked")
		else
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "User [ "..name.." ]"..msg.from.id.." Public Banned (Spamming)")
		  send_large_msg("channel#id"..msg.to.id, "User [ "..name.." ]"..msg.from.id.." Public Banned (Spamming)")
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "User [ "..name.." ] ( @"..username.." )"..msg.from.id.." Public Banned (Spamming)"
